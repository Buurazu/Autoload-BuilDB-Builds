_G.AutoloadBuilDB = _G.AutoloadBuilDB or {}

function AutoloadBuilDB:points_at_level(level)
	local bonuspoints = 2 * math.floor(level/10)
	return level + bonuspoints
end

function AutoloadBuilDB:autoload(loc)
	--avoid any and all potential crashes
	if not managers.statistics or not managers.experience or not managers.job or not managers.multi_profile or not managers.skilltree then
		log("[ABB] Cancelling autoload attempt; game managers were not set up")
		return
	end
	...
	--really stupid way to load buildb in a mod that isn't buildb (shouldn't be necessary because I load builds as late as possible?)
	if not BuilDB then
		local ourModPath = ModPath
		ModPath = ModPath .. '../BuilDB/'
		dofile(ModPath .. '../BuilDB/lua/_buildb.lua')
		log("[ABB] BUILDB WAS MISSING; loading it at mods/BuilDB/lua/_buildb.lua")
		ModPath = ourModPath
	end
	if not BuilDB then 
		log("[ABB] BuilDB not found!")
		return
	end
	...
	log("[ABB] Checking for BuilDB builds to autoload... (" .. loc .. ")")
	...
	for i, v in ipairs(managers.multi_profile._global._profiles) do
		...
		if (autoloadable[name] and AutoloadBuilDB:points_at_level(bestlevels[name]) > pointsused) then
